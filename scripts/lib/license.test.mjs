...
describe("verifyKey refuses what it should", () => {
  const cases = [
    ...
    ["two segments", "arm1.abc", "expected three dot-separated parts"],
    ["an unknown prefix", valid.replace(/^arm1\./, "arm2."), "unknown key format 'arm2'"],
    ["an empty payload", "arm1..abc", "empty payload or signature"],
  ];
...
describe("the Swift twin", () => {
  // apps/apple/Armada/License.swift makes the same four checks in the same
  ...
  const swift = readFileSync(
    join(import.meta.dirname, "../../apps/apple/Armada/License.swift"),
    "utf8",
  );
