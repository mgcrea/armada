...
//
// The app cannot phone home. scripts/audit-network.sh asserts Armada reaches no
// network beyond the opt-in update check in Updates.swift — so a key has to prove
// itself from nothing but the bytes it carries and a public key compiled into the
// binary. That rules out every activation server and leaves exactly one shape:
// sign a payload here, verify it there, never ask anyone.
//
...
// enough that a key is pasted or dropped and never typed — LicensePane is
...
// feeds is a few readable lines in EntitlementGate.swift and the source is public, so
...
//
// Dependency-free (node builtins only). The format is identical to bastion's and
// cupertino's but for the prefix, and deliberately so: the Worker in apps/api
// implements the same bytes against WebCrypto, which is why the signature
...
export const KEY_PREFIX = "arm1";

/**
 * The public half of the signing key, in base64url.
 *
 * Lives here AND in apps/apple/Armada/License.swift, because the app cannot
 ...
 */
export const PUBLIC_KEY = "OTc_jH9ZwYjtYaU0_MP74gsiupje0ZafXTmTiRafI0E";
 ...
 * mostly that one sentence. The Swift twin in apps/apple/Armada/License.swift
