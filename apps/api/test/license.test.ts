...
import {
  KEY_PREFIX as NODE_PREFIX,
  generateKeypair,
  mint as mintOnNode,
  verifyKey,
} from "../../../scripts/lib/license.mjs";
import { KEY_PREFIX, mint, ulid } from "../src/license";
...
describe("mint", () => {
  ...
  // Byte-identity already implies this, but a wrong prefix is the one drift
  // that would be copied in from a sibling app's Worker, and it deserves to
  // fail with its own name rather than as two long strings that differ.
  it("uses Armada's prefix, the same one the Node implementation parses", async () => {
    expect(KEY_PREFIX).toBe("arm1");
    expect(KEY_PREFIX).toBe(NODE_PREFIX);
    const minted = await mint({ ...fixed, privateKey: keys.privateKey });
    expect(minted.key.startsWith("arm1.")).toBe(true);
  });
