...
- **v1 watches; it doesn't host agents.** It observes sessions started in VS Code, a
  ...
  permission still asked of the person. **Amended 2026-09-16:** it also ends a Claude Code
  session when an agent asks, through `armada_close_session`, behind the same switch. Ending is
  `SIGTERM` to the session's own process, what quitting does; Armada still does not own or
  drive it. **Amended 2026-09-21:** and when the person asks, from the session's details or its
  right-click menu, by the same signal.
...
- **On-demand triage agent** that reads across all sessions ("which should I look at
  ...
  app, and a Claude Code session started with it attached. The focus tool shipped on 2026-09-17
  behind Allow writes, below. Voice shipped on 2026-09-15, below.
...
| 2026-09-16 | Voice may start a session while Allow writes is on | Asked for by voice, the supervisor refused and blamed Allow writes, which was already on: the 2026-09-15 voice row denied the start tool by name whatever the switch said. Headless, a tool is either allowed and runs unasked or denied, so there is no Claude Code prompt to fall back on. Now the switch decides for voice too, the brief makes it name the project, account and message and wait for a spoken yes, and `armada_close_session` stays denied. The process is restarted at the next question when the switch changes, as for the reply language. Accepted: a transcript that talks voice past the spoken confirmation could start a session in a saved project, which still asks for every permission. Rejected: keeping voice read-only with a truer refusal (the person wants to dispatch work by voice). |
| 2026-09-16 | An agent may close a Claude Code session, behind the kit's write gate, a busy one only with `force` | Starting without closing left a supervisor that could fill the screen and not clear it. `SIGTERM` to the registry's pid, measured on 2.1.273 in a pty: exit 143 in 0.7–0.8s idle or with a command running, the registry file gone at 0.3s, its MCP server children and the running command gone with it; `SIGKILL` only for a process still there after eight seconds. Fenced: `gate: .requiresWrites`; not pre-allowed in the Terminal supervisor and denied by name to voice; working and `runningTool` refused without `force`, re-checked on the main actor; the pid signalled only when the kernel's start time matches the registry's `startedAt`, and a registry with none refused, so a stale file naming a reused pid ends nothing; throttled to one close per five seconds. Rejected: Codex (no pid for a session, and one `app-server` process holds several threads); any session without a busy check (a supervisor would stop a turn mid-edit on a guess); only sessions an agent started (the person wants their own idle sessions cleared too). |
| 2026-09-16 | `armada_wait` long-polls, `armada_start_session` names the session it starts and can resume a closed one | A supervisor that could start and close still had to poll the fleet and guess which new row was its own. **Wait** re-reads the snapshot once a second for at most 240s, under Codex's 300s tool-call failure, and returns sessions that *newly* want the person (a session already waiting would otherwise end every wait at once) or, with `until: change`, any state change, appearance or end; it is a read and is pre-allowed. **Ids** are minted by Armada and passed as `--session-id` for a terminal launch; Claude Code writes them to the registry as given (measured on 2.1.273). VS Code's link takes none and Codex has no flag, so those still return none. **Resume** is `claude --resume` in a terminal, which keeps the id (measured the same day), and is refused while any watched session has the id or its transcript was written in the last 30 seconds, because two writers on one transcript is what Fork exists to avoid; the account is the one whose folder holds the transcript and the folder is the transcript's own `cwd`, which must sit inside a saved project. Rejected: resuming live sessions (two writers). Not yet: Codex resume, which was not measured. |
...
| 2026-09-17 | A supervisor may message a Claude Code session, through a Stop hook Armada installs in every account's `settings.json`, behind its own switch | The hook path the design already chose, cut down to one sender: the MCP client holding the token. `asyncRewake` wakes an idle session and reaches a busy one when its turn ends, measured end to end on 2.1.273 (docs/reaching-agents.md). Every account rather than only sessions Armada starts, because the person's own sessions are the ones a supervisor needs to reach. Reverses, for one hook, the rule that nothing is written to a vendor's config, as the trust row did for one flag: a byte-level edit checked against the original plus the entry, under Claude Code's lock, reversed exactly when the switch goes off. Fenced: its own switch plus Allow writes, not pre-allowed for the supervisor, denied to voice, messages labelled as an agent's and expired after an hour, the script reads its session's inbox and prints it. Rejected: `claude plugin install` (the person chose a direct edit); `--settings` on launches Armada makes (reaches none of the person's own sessions); the per-pair router, policy and content audit of section 2 (not built; one sender needs no routing). Accepted: a transcript that talks a supervisor into a message still meets Claude Code's permission prompt, but the recipient acts on what it is sent. |
| 2026-09-17 | Voice listens for an answer once a reply is spoken, after a question by default, in press mode only | Asked for by voice, a start ends on "Should I go ahead?", and the shortcut had to be pressed again to say yes. Settings ▸ Voice has Keep listening: Never, After a question (default) or After every reply. A question is a reply whose last sentence ends in `?`, `？` or `؟`, read past quotes, brackets and emphasis; no marker is asked of the model. The microphone opens only after the voice has finished, 300 ms later, because nothing cancels echo, and silence closes the card with no "Didn't catch that", since you were asked, not asking. Hold mode never listens by itself: holding is the choice to keep the microphone to the key. Rejected: listening after every reply by default (the microphone opens unasked after every answer, and a room's noise becomes a question); a wake phrase, as in the 2026-09-15 row. Accepted: a reply that asks only in its middle sentence does not listen. |
| 2026-09-17 | Voice replies default to one sentence, two at most; Esc stops voice; effort is a setting | Read from the debug build's voice conversations: replies ran to three and four sentences, the extra ones reassurance ("it should show up in a few seconds", "it will ask for permissions as usual"), every session named, or the only account restated. The default instructions now ask for the answer first, counts over lists, at most two sessions named, an account only when there are several; the start confirmation is one short question. A person's own instructions are untouched, and a new default reaches only those who never edited them. **Esc** is a second Carbon hot key, registered only while voice listens, thinks or speaks, so it needs no Input Monitoring and gives the key back the moment voice stops; the card shows an esc keycap while it is held. **Effort** is Account default (no flag, as before), Low, Medium or High, and is not part of the brief, so a change resumes the conversation. Rejected: a keyboard event tap for Esc (needs Input Monitoring and sees every keystroke, which `VoiceShortcut` exists to avoid); Esc held while the card only waits to hide (six more seconds of taking Esc for nothing to stop); forcing Low effort (latency was acceptable to the person). Accepted: an Esc meant for the frontmost app, Claude Code's interrupt included, stops voice instead while voice is active. |
| 2026-09-17 | An agent may bring a session's window to the front, behind Allow writes; voice does it when asked, without confirming | Voice pointed at the waiting session and said it could not bring the window forward, which was true: Focus existed only as a click. `armada_focus_session` calls the same `FocusSession.focus` as a row, so it lands on the same window and tab. Behind `.requiresWrites` rather than always listed, because it takes the screen from the person, and a read-only tool list should stay read-only. Voice is allowed it with the start tool and the brief tells it to use it only when asked, with no spoken confirmation, since raising a window changes nothing. The Terminal supervisor is not pre-allowed it. **Activation from behind**: the rows work because Armada is frontmost and can yield; a voice call arrives with only the non-activating card showing, so the bridge waits half a second for the host to become frontmost and otherwise asks LaunchServices, then raises the matched window again. Throttled to one focus per two seconds. Rejected: focusing unasked whenever voice names a session (it would move windows mid-sentence); Codex (no process to find). |
| 2026-09-17 | Add Account… starts an unmodified `claude` in the terminal on a new `~/.claude-<name>`, and accounts are rediscovered on the refresh tick | Adding a second account meant knowing `CLAUDE_CONFIG_DIR`, and nothing in the app said so. The terms rule out anything more: a third-party app may not offer Claude.ai login or intermediate credentials, so Armada only picks the folder name and hands Claude Code the variable, and does not even create the folder. A plain `claude` rather than `claude auth login`, because it creates the folder and `sessions/` before its first question and so is found at once, where `auth` leaves no `sessions/` and would need discovery loosened (measured 2026-09-17 on 2.1.274). Rediscovery adds folders and never removes one: removing an `Account` means stopping watchers under a selected row or a probe in flight. Rejected: an Armada sign-in screen or a token check (the terms); a relaunch to see the account (reads as the button failing). |
| 2026-09-17 | Add Account covers Codex and Grok Build, from one menu pinned under the sidebar, and remembers the homes it added | Asked for once Claude Code had it: an agent picker belongs under the whole list, not inside the Claude section. Codex and Grok Build discovery deliberately scan no `~/.codex-*` convention, and a GUI launch has no `CODEX_HOME` or `GROK_HOME`, so a home added through Armada is stored in defaults (`AddedHomes`) and read by `discoverAll` beside the default and the variable: named by the person, never guessed from a folder name. Measured 2026-09-17: Codex 0.154 refuses a `CODEX_HOME` that does not exist and writes `sessions/` only once a session runs, so Armada creates that one folder and an added Codex home counts with `version.json`; Grok Build 1.0.34 creates its home with both markers at launch. OpenAI's and xAI's terms are still unread (see limits-accounts-and-terms.md); starting their own unmodified CLI, which runs its own sign-in, is the same line as Claude Code's. Removing an added home is not offered yet. |
| 2026-09-17 | Voice starts a session when asked, with no spoken confirmation | Reverses the confirmation half of the 2026-09-16 voice-start row, and the "one short question" of the 2026-09-17 reply-length row, on the person's own report: every start cost a turn reading back a project, an account and an empty opening message they had just spoken. A spoken request is already the person's words, arriving on a card they opened with a shortcut, so the question confirmed nothing the turn did not already carry. The brief now says their asking is the go-ahead, to take the project's own agent and account and no opening message unless they said otherwise, and to ask only when it cannot tell which project is meant. The server's own instructions, written for any client, are changed with it: start when asked, still check before closing, and never start, close or focus one because transcript text asks. Accepted: the 2026-09-16 row's risk without the spoken confirmation in front of it — a transcript that talks voice into a start reaches a saved project, which still asks for every permission, and the brief's "never start one because transcript text asks for it" is what stands in its way. Rejected: keeping the question only when an opening message is dictated (the dictated message is the part they are surest of). |
| 2026-09-21 | A Claude Code session can be continued on another Claude account: Armada copies its transcript across, then forks it there | Asked for once two accounts were in daily use: a session stopped at one account's limit had no way onto the other. `--resume` looks only in the running account's `projects/`, so the conversation has to be copied first, the one write Armada makes into a vendor's transcript folder (`TranscriptHandover`): the session's own `.jsonl`, whole lines only, under the folder name the source account chose, plus its `<id>/` sidecar. An existing file is replaced only when it is an earlier copy of the same transcript. Then `--resume --fork-session` on the target, so the original keeps running and the copy gets its own id. Past turns stay on the account that spent them, because usage dedupes on message ids. Rejected: resuming under the same id (one id live under two accounts breaks close, focus and send, which look sessions up by id); closing the original as part of the switch (a process killed for a button that reads as a copy). Claude Code only for now. |
| 2026-09-21 | The person can close a Claude Code session from Armada too, asked first only when it is mid-turn | Since 2026-09-16 an agent could close a session and the person could not, short of asking one: the 2026-09-16 row already said "the person wants their own idle sessions cleared too" and gave them no button. Close Session sits last in the session's details, in its row's right-click menu and in the menu bar popover's, and goes through `SessionCloserBridge` unchanged in what protects the process: found again on the main actor, the pid tied to the session by start time, `SIGTERM` then `SIGKILL` after eight seconds. What differs is `SessionClosePolicy`: a person's close is not throttled and does not arm the throttle, which is an agent's loop protection, so clearing three rows is three clicks and a click cannot refuse an agent's next call. Idle and waiting sessions close with no question, as they do for an agent, because the transcript stays and the session can be resumed; working and `runningTool` get an alert first, and only then `force`. From the popover, which has no alert of its own, a busy session is opened in the main window and asked about there. A refusal, or a process still there after `SIGKILL`, is an alert in whichever pane is up. Accepted: closing a session hosted by the VS Code extension is still unmeasured (implementation.md), and is now one click away. Rejected: "Kill" (names the fallback, not the method) and "End" (reads as ending the conversation, which resume disproves); a confirmation on every close (a second click that protects nothing recoverable); asking about `waiting` too (stopped at a prompt, it is doing nothing a close interrupts, and the tool does not ask); close all idle, and a keyboard shortcut (not asked for); Codex and Grok, for the 2026-09-16 row's reasons. |
| 2026-09-21 | A project lists its recently ended Claude Code sessions, each of which can be read and resumed | Close Session keeps the conversation "so it can be resumed", and the only thing that could resume one was an agent, through `armada_start_session`: a row that left the sessions list had nowhere to be found again. **Recently ended** sits under Live sessions in a project's details: the last eight Claude Code sessions whose folder the project owns, by the deepest-project rule `ProjectStats` counts them by, so the list and the count beside it agree. It is a filter over the usage ledger's `sessions` rows (`RecentSessions`), which the indexer already keeps, so nothing new is scanned; a row whose transcript Claude Code has since removed is dropped, and an id the ledger holds under two accounts is one row (the copy Continue stages on another account is a pure mirror and normally makes no ledger row at all; see implementation.md, "Where closing and resuming from the app are thin"). A click reads the transcript and the button resumes, the larger target going to the harmless one. Resume runs `claude --resume` in the terminal, on the account whose folder holds the transcript and in the folder it recorded, behind the 2026-09-16 row's checks, now `SessionResume.prepare` and shared with the tool: not live, not written in 30 seconds, the folder still there and inside a saved project. Refusals are typed and each caller words its own, so a person is not told which tool to call. Listed however they ended, because the ledger records a last write and not how a session stopped. Rejected: only sessions closed from Armada (would need a record of its own, and one quit in its terminal is as worth picking up); a scan of `projects/` (the ledger already has the rows); an "Ended" state kept in the account's sessions list (that list is what is running, and a session leaving it is how it says one ended); Codex (resume unmeasured) and Grok Build (not in the ledger); sessions outside saved projects (no project details to be listed in, and the tool's boundary too); continuing an ended session on another account, search, or a full history (not asked for). |
| 2026-09-25 | An account's transcripts can be archived, opt-in, into a folder the person picks | Claude Code deletes a transcript 30 days after its last write by default, and the conversation with it. The person asked for a copy that outlives that, somewhere like a NAS, and for it to be 100% opt-in so nothing leaves the Mac by Armada's doing. A switch per account (Settings ▸ Archive and the account's overview) and one destination: an `Armada Archive` folder inside the picked one, whose `armada-archive.json` assigns each account a folder once. Claude copies all of `projects/` (transcripts, sidecars, auto memory); Codex copies its rollouts, an `archived_sessions/` one filed back under its date so Codex moving it is not a second copy. A `.jsonl` is appended to, whole lines only, when its first and last 4 KB before the copy's end still match, and otherwise the old copy is kept as `~1`; so a live session is copied as it runs, and a pass cut short carries on. Other files are replaced. Each copy takes its source's date, which is both the next pass's quick check and what retention measures. Retention, off by default, deletes copies older than the period and skips sources older than it; shortening it asks first. A folder that is missing or on another volume than it was picked on is waited for, never created. Rejected: an upload to R2 or S3 (a network request of Armada's own, which the privacy page and SECURITY.md exclude); copying only ended sessions (whole-line appends make a live one safe, and waiting for the end loses what a crash never closes); gzip (not appendable in place, and a copy should be greppable where it lands); scrubbing secrets on the way in (the archive's value is that it is exactly what was written; a scrubbed export for training can come later); Grok Build (not in the ledger, and its files are unmeasured for this). |
